...
for i=3:nImagenes
    ...
    for a=1:(max(max(superpixeles))+1)
        ...
        imagenSuperpixel(row,column)= etiquetas.streaks(a);
    end
    ...
    for j=(1+tamBloque/2):round((rand+0.1)*10):(size(borde,1)-tamBloque/2)
        for k=(1+tamBloque/2):round(rand*10):(size(borde,2)-tamBloque/2)
            ...
            if(borde(j,k)==255)
                ...
                %Se sacan las etiquetas de streaks existentes
                %en el bloque
                valores_streaks(1:64,1:64)= imagenSuperpixel((j-tamBloque/2)+(0:tamBloque-1),(k-tamBloque/2)+(0:tamBloque-1));
...
for j=1:numFolds
    ...
    nStreaks= length(indicesMelanomaStreak);
    melanomaBenigno= find((eval(strcat('etiquetasfold',num2str(j))))==0);
    [indicesMelanomaBenigno,idx] = datasample(melanomaBenigno,nStreaks);
    
    %Se actualiza el nuevo array de etiquetas del fold
    indicesMelanomas= [indicesMelanomaStreak indicesMelanomaBenigno];
        switch j
            case 1
...
%Resultados: Tasa de error, PFA, PM
predictedLabelsTest_noCat= completepredictedLabelsTest=='1';
tasa_error= sum(completepredictedLabelsTest ~= categorical(completeY))/numel(completeY);
pFA= sum((predictedLabelsTest_noCat==1) & completeY==0)/(sum(completeY==0));
pM= sum((predictedLabelsTest_noCat==0) & completeY ==1)/(sum(completeY==1));

%Curva ROC
[Xroc, Yroc]=perfcurve(completeY,completeScores(:,2),'1');
plotroc(completeY,predictedLabelsTest_noCat);
plot(Xroc,Yroc);
